...
public class BoundaryTestCases {
	...
	public void validateBoundaryConditions() {
		// Minimum value
		given().queryParam("age", 0).when().get("https://api.example.com/users").then().statusCode(200);

		Log.info("Minimum boundary condition validated successfully.");

		// Maximum value
		given().queryParam("age", 150).when().get("https://api.example.com/users").then().statusCode(200);
